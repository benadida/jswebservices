/*
 * Main Library for the JS Web Service Shim
 *
 * Think of this as the user-agent making calls into JS Web Services
 */
...
var JSWebService = (function() {
  // listen for all message events
  window.addEventListener('message', function(e) {
    var service = SERVICES[e.origin];

    // security check
    if (service.origin != e.origin) {
      console.log("origin mismatch, buh bye");
      return;
    }

    // ready!
    if (!service.isReady) {
      if (e.data.type != 'jswebservice-service-loaded')
        return;

      service.isReady = true;
      service.frame.contentWindow.postMessage({"type": "jswebservice-client-ready"}, service.origin);

      // extract callback and null it
      var callback = service.initCallback;
      service.serviceCallback = null;

      // set up the port
      service.port = new JSWebServicePort(service);
  ...
  var instantiate = function(url, callback) {
    var origin = URLParse(url).normalize().originOnly().toString();

    // create an IFRAME onto the URL
